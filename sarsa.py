...
# ============== LIGHTWEIGHT MAZE & ENV (tối giản, tương thích env của anh) ==============
class Maze:
...
class MazeEnv:
    ...
    - Reward đã chỉnh trong env của anh (Cách 1 + shaping Cách 3).
    ...
    def __init__(self, maze: Maze):
        ...
        # đồng bộ gamma với env mới (nếu chạy độc lập)
...
class SarsaAgent:
    def __init__(
        ...
        gamma: float = 0.98,       # discount (khớp env.GAMMA)
        epsilon_start: float = 1.0,
        epsilon_end: float = 0.05,
        epsilon_decay_episodes: int = 800  # decay dài một chút
    ):
...
def train_sarsa(
    ...
    for ep in range(1, episodes + 1):
        ...
        for _ in range(max_steps_per_ep):
            ...
            eps_next = agent.epsilon_by_episode(ep)  # same ep (on-policy)
            a_next = agent.act(s_next, eps_next)
        ...
        success_window.append(1 if s == env.goal else 0)
...
def evaluate_greedy(env: MazeEnv, agent: SarsaAgent, episodes: int = 40, max_steps_per_ep: int = 400):
...
if __name__ == "__main__":
    ...
    # ======== TRAIN ========
    t0 = time.time()
    agent, rewards, steps = train_sarsa(
        env,
        episodes=EPISODES,
    ...
    # ======== SAVE Q-TABLE ========
    out_q = "/Users/haiduong/Desktop/Maze/qtable_12x12_sarsa.npy"
    ...
    plt.xlabel("Episode"); plt.ylabel("Total Reward"); plt.title("Training Reward (SARSA)"); plt.legend()
